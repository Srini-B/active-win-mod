...
const wchar_t = require('ref-wchar-napi-lite');
...
function getFileVersionString(block, name) {
	...
	let data;
	if (result) {
		const buffer = ref.readPointer(value, 0, size.deref() * wchar_t.size);
		data = wchar_t.toString(ref.reinterpretUntilZeros(buffer, wchar_t.size));
	}
...
function windows() {
	...
	let windowTitle = wchar_t.toString(windowTextBufferClean);
